...
-- Adiciona o toggle do Anti AFK
local AntiAFKToggle = Tabs.Others:AddToggle("AntiAFKToggle", {
    Title = "Anti AFK",
    Default = true, -- Define o toggle como ativado por padrão
    Callback = function(value)
        -- Você pode adicionar aqui a lógica que você quer que aconteça ao ativar/desativar o Anti AFK
        if value then
            print("Anti AFK ativado!")
            -- Lógica para ativar o Anti AFK
            -- Por exemplo, mover o personagem levemente a cada alguns segundos
            while value do
                game.Players.LocalPlayer.Character:MoveTo(game.Players.LocalPlayer.Character.HumanoidRootPart.Position + Vector3.new(0, 0.1, 0)) -- Move o jogador um pouco
                wait(5) -- Aguarda 5 segundos antes de mover novamente
            end
        else
...
-- Ativa o Anti AFK imediatamente
AntiAFKToggle:SetValue(true) -- Mantém o toggle ativado
...
-- Função para teletransportar para Mobs
...
local function autoFarm()
    while autoFarmEnabled do
        -- Troque o nome 'NomeDoMob' pelo nome real dos Mobs no seu jogo
        local mobs = workspace:GetChildren()
        for _, mob in ipairs(mobs) do
            if mob:IsA("Model") and mob.Name == "NomeDoMob" then
                teleportToMob(mob)
                wait(1) -- Atraso entre teleportações, ajuste conforme necessário
            end
        end
        wait(1) -- Atraso entre buscas, ajuste conforme necessário
    end
end
...
local AutoFarmToggle = Tabs.Combat:AddToggle("AutoFarmToggle", {
    ...
    Default = false,
    Callback = function(value)
        autoFarmEnabled = value -- Atualiza o estado do toggle
        if value then
            spawn(autoFarm) -- Inicia a função Auto Farm em uma nova thread
        end
    end
})

-- Substitua 'NomeDoMob' pelo nome dos Mobs que você deseja teleportar
